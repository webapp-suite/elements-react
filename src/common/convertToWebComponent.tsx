...
const createEventPropName = (eventName: string) =>
        ...
        (props: T & WebComponentPropTypes, wcRef: RefObject<DomRef>) => {
            ...
            const regularProps = regularProperties.reduce((acc, name) => {
                if (rest.hasOwnProperty(name)) {
                    ...
                    return {
                        ...acc,
                        [mapProperties[name] ?? camelToKebabCase(name)]:
                            isObject(rest[name])
                                ? JSON.stringify(
                                      convertObjectKey(
                                          rest[name],
            ...
            const booleanProps = booleanProperties.reduce((acc, name) => {
                if (rest[name] === true || rest[name] === 'true') {
                    return {
                        ...acc,
                        [mapProperties[name] ?? camelToKebabCase(name)]: true,
                    };
                }
                return acc;
            }, {});
            ...
            useEffect(
                ...
                eventProperties.map(
                    (eventName) => rest[createEventPropName(eventName)],
                ),
            );
            ...
            const tagNameSuffix: string =
                getEffectiveScopingSuffixForTag(tagName);
